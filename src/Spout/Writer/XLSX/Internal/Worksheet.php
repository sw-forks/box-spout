...
EOD;
    ...
    public function getColWidths($colWidths)
    {
        if (!count($colWidths)) return '';

        $result = [];
        $result[] = '<cols>';
        foreach ($colWidths as $key => $col_width) {
            $result[] = '<col min="' . ($key + 1) . '" max="' . ($key + 1) . '" width="' . $col_width . '" bestFit="1" customWidth="1"/>';
        }
        $result[] = '</cols>';
